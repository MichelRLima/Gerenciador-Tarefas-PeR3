...
const CriarTarefa = ({ handleClose, tarefas, setTarefas }) => {
  const [idTarefa, setIdTarefa] = useState(1); // Inicializa o ID com 1
  ...
  useEffect(() => {
    // Calcula o próximo ID baseado nas tarefas existentes
    if (tarefas.length > 0) {
      const maxId = Math.max(...tarefas.map(tarefa => tarefa.idTarefa));
      setIdTarefa(maxId + 1);
    }
  }, [tarefas]);
